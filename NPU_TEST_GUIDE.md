...
2. 前 10 条过切样本的 `db_range` 全在 53-92dB——**强瞬态锁死参考值的特征吻合**
   （VAD 后只剩 0.6-1.1s 的瞬态，且 >min-dur 0.3 会进训练集）。
   实测 `--ref-percentile 95` 后：过切 39/19/21 → 31/17/15（-21%），
   p50 保留升至 0.35/0.53/0.59，切掉比例 63/50/49% → 57/46/40%。
   改善有限但方向对；**是否默认 95 请听完再定**，抽听路径：

   | 原 wav | VAD 后 wav |
   |---|---|
   | `/home/ma-user/work/dataset/duplex_whj_data/<集>_wavs/<uttid>.wav` | `/tmp/v2_wav_smoke3/<ds>/<uttid>.wav` |
   
   uttid 前 10 见 `finetuning/data_v2_smoke3/suspicious_vad.jsonl`（dataset/keep/db_range
   字段齐全）。注意：部分「过切」可能只是 turn 边界静音长（data_report 说 turn
   中位 5.2s，切掉一半静音属正常），区分标准是**剩下来的是不是瞬态**。
