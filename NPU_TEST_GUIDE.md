...
**这个坑已经从工具层堵上了**（`fd3ab08` 之后）：那个只含 encoder 的数是我的
基准写错了 —— 它复用一条已经 load 好的音频，把最大的一块（读盘 + 重采样）
排除在外了。冒烟现在**额外报一行端到端耗时**（拿 4b 的真实产物逐条 load + 抽），
直接给出全量该看的小时数，不用再人工换算：

```
✅ SPK_MEL_THREADS 实测最优 = 1（encoder 前向 4.6 ms/条；不设时 98.8 ms/条）
✅ 端到端（含 librosa.load + 重采样）5.8 ms/条 → 43.5w 条约 **0.7h**
```

开发机 5.8 ms/条（本地 SSD + 24k 源）vs 昇腾 66.3 ms/条（SFS 网络盘 + 16k 源
要重采样）—— **差 11 倍，全在 I/O 与重采样上，与 encoder 无关**。
所以昇腾侧要提速应该往「把 wav 挪到本地盘」或「源就存 24k」上想，
调线程数没用（那边只差 13%）。
...
### 十三轮（开发机）：修 NPU 侧报的「冒烟假通过」+ 端到端计时（2026-08-16）

1. **【高】冒烟会在 spk 文件有问题时报假通过** —— NPU 侧十二轮记的
   「0 字节 spk 文件会让冒烟静默跳过且报告仍打全部通过」属实，**本机复现**：
   `--spk_file` 指向空文件跑 `--stages 5`，一项没验却打「失败阶段: 无 ——
   全部通过 ✅」且退出码 0。

   根因：main 里用 `raise SystemExit` 当「阶段失败后中止后续」的**控制流**，
   配 `except SystemExit: pass`。而阶段函数内部的**真** SystemExit
   （`open_spk_memmap` 的行数硬校验）被同一个 except 吞掉，`failed` 里什么都没加。

   改成自有的 `_Abort` 异常做控制流，`SystemExit` 一律当真错误计入 failed。
   另加 `--spk_file` 前置检查（存在性 + 大小是 4096 的正整数倍）。
   实测：空文件 → 退出码 1「失败阶段: 硬校验」并打出原因；
   不存在/被截断 → 退出码 2 并说清楚该干什么。

   **这是测试工具最坏的一类失败**（报假通过），排查优先级高于任何功能缺陷。

2. **线程基准把最大的一块排除在外了**：它复用一条已 load 好的音频，
   只量 encoder 前向。所以报「43.5w 条约 0.5h」而真实是 8h。冒烟现在额外
   报一行**端到端**耗时（拿 4b 的真实产物逐条 load + 抽），见 §16.2(1)。

3. 由 NPU 侧数据得到的一个结论：**昇腾侧抽 spk 的瓶颈是 I/O 与重采样，
...
**算子级形状统计**（TorchDispatchMode 记录每个 aten 算子的输入形状，
40 步，样本按真实时长分布造，帧数 5-103 共 57 种）：

| | sub-talker 内 | 主路径 | 合计 |
|---|---|---|---|
| 帧数不分桶 | **2574** | 731 | 3305 |
| 帧数分桶 64 | **172** | 833 | 1005 |

- **sub-talker 形状降 93%**，总降 70%
- 修前 sub-talker 占全部动态形状的 **78%** —— 是压倒性的主要来源

> ⚠️ 这个测量对**数据的形状多样性**极其敏感：首次用 10 条短样本测时降幅只有 1%，
> 因为那批数据本身只有 2 种 N_frames，没得可降。**用小样本测分桶效果会得出
> 完全错误的结论**，必须按真实时长分布造数据。
...
**2026-08-18 NPU 实测回填（leak_probe 实验）**：

- **假说成立**：分桶后瞬时水位稳定——step 523→1177 间 mem 54.24→54.25GB
  （此前 7MB/步持续爬升消失）。斜率数字从 +18 降到 +7.8MB/步 是「自 step20
  平均」被早期编译拖高的假象，不是真实爬升
- **但 step ~1178 仍 OOM**：59.85GB 已分配时请求 46MB 失败——不再是泄漏，
  是**峰值余量不足**（基线水位 54GB + 单步峰值 ~6GB > 61GB 上限）
- **对策**：batch 8→4 + ACLNN_CACHE_LIMIT 300→100（形状收敛 93% 后缓存需求
  大降），已从 step35000 续跑 2-epoch 剩余 73k 步（约 10-12h）
- 结论对应表格第二行（假说成立，已修），附带的峰值问题由降 batch 解决

**顺带**：`SUB_TALKER_FRAME_BUCKET` 默认已是 64，上面显式写出来只是为了让
对照实验（设 0）好写。若要测不分桶的基准：`SUB_TALKER_FRAME_BUCKET=0`。

### 十五轮（NPU 侧）：sub-talker 分桶修复验证 + 峰值余量问题（2026-08-18）

1. 远端三连修（2c17ac0/440f15a/d74b6b0）：sub-talker 帧数分桶（头号嫌疑：
   动态形状 93% 来自 sub-talker）、日志附 mem 水位+斜率、§21.4 判定实验
2. NPU 实测：分桶后**逐步增长消失**（水位 54.25GB 数百步平稳），假说成立；
...
### 21.5 斜率指标已修（首版会误报"仍在漏"）

NPU 侧回填里那句「+18 降到 +7.8MB/步 是自 step20 平均被早期编译拖高的假象」
说得对 —— 是**我这个指标的缺陷**，不是判读问题。首版用「自基准点的累计平均」，
早期一次性的编译抬升会被长期摊进均值里，水位早就平了它还在报正值。

已改成**最近 20 个日志点的最小二乘斜率**。用两种形态回放验证：

| 场景 | step | 旧（累计平均） | 新（窗口拟合） |
|---|---|---|---|
| 已修好（早期抬升后平稳） | 820 | +4.50 ❌ | **+0.00** ✅ |
| 已修好 | 1220 | +3.00 ❌ | **+0.00** ✅ |
| 真在漏 7MB/步 | 1220 | +7.00 ✅ | **+7.00** ✅ |

新指标在"已修好"时干净归零，在"真在漏"时不丢灵敏度。日志格式变成：

```
Epoch 0 | Step 1200 | Loss: 2.9013 | mem 54.25GB (近 20 点 +0.01MB/步)
```

### 21.6 峰值余量不足：batch 8→4 之外还有一条路（若 batch4 仍紧再考虑）

现状是**基线水位 54GB + 单步峰值 ~6GB > 61GB 上限**。降 batch 直接砍激活，
有效但吞吐减半。

54GB 基线里绝大部分是**参数 + 梯度 + AdamW 动量**（fp32 下约
6.8 + 6.8 + 13.6 ≈ 27GB，再叠模型 CPU 副本与碎片），**降 batch 动不了它**。
若 batch4 仍紧，按代价从低到高：

| 手段 | 省什么 | 代价 |
|---|---|---|
| **梯度检查点** | 激活（峰值大头），可保持 batch 8 | 重算前向，约慢 20-30% |
| `--dtype bf16` | 静态 25GB→12.7GB | Adam 动量也变 bf16，收敛风险（§8.5） |

`supports_gradient_checkpointing = True` 已在 modeling 里（:492/:523），
talker 支持，但**训练脚本目前没有接这个开关** —— 真要用我再接，改动很小。

**batch4 那轮先跑着，别为这个重启。** 只是记下来备用。

## 22. 模型不稳 / 生成断掉 —— 分块训练丢了 optimizer 动量（2026-08-18）

### 22.1 先排除的

| 怀疑 | 结论 |
|---|---|
| `max_new_tokens` 太小 | ❌ 默认 2048（≈170 秒），不是长度上限 |
| 推理流式喂文本、训练不流式 | ❌ `non_streaming_mode=True`（默认）下推理把正文与 `codec_pad` 相加放在 codec 之前，与 collate 逐格一致。**但 `non_streaming_mode=False` 会走逐帧喂文本的分支（modeling:1718），与训练不匹配 —— 别改这个参数** |
| codec 本身坏了 | ❌ 原始音频 encode→decode 实测：10.72s → 10.72s，波形相关 **0.922** |

### 22.2 头号原因：每块热启都把 AdamW 动量清零

`_save` 不存 optimizer 状态，脚本里也没有 lr_scheduler（lr 余弦是 wrapper 每块传
不同 `--lr` 做的）。所以**每块热启都是全新的 AdamW（m=0, v=0）**。

刚重置时二阶矩估计还没建立，前若干步的更新量接近 `lr·sign(g)`，远大于稳态 ——
**每块开头都挨一次冲击**。5000 步一块、10 万步总量就是 **20 多次**。

开发机实测（同一权重起步，块 2 的前 5 步）：

| 步 | 不接状态 | 接上状态 |
|---|---|---|
| 0 | 1.4893 | 1.4893（同权重，必然一致） |
| 1 | **4.2567** | 2.1001 |
| 2 | 3.2304 | 1.5473 |
| 3 | **5.2259** | 2.3089 |
| 4 | 2.3273 | **1.1227** |

不接状态时 loss 在块开头剧烈反弹，接上后平稳下降。这与「模型很不稳、
生成到一半就断、调 temperature 没用」的表现一致 —— 不是采样问题，是权重
被反复冲击。

### 22.3 已加 `--optim-state`（启动脚本 `OPTIM_STATE`）

块末原子写、块初读（`os.replace`，崩在写一半不会留坏文件）。
fp32 下约 **14.3GB**（实测），覆盖写只占一份。

```bash
OPTIM_STATE=$E/runs/v2_full/optim.pt \
MODEL_PATH=... SKIP_PREPARE=1 bash run_v2_npu_debug.sh
```

留空则不存不读，行为与以前相同。已验证 `accelerator.prepare` 之后
`load_state_dict` 的 key 对得上（404 个参数带动量、`step` 计数接上、
一阶动量逐位一致）。

### 22.4 还有两个次要嫌疑（若接上动量仍不稳再查）

1. **训练量不够**：当前 35k / 约 108k 步（2 epoch），才三成。而且此前那 35k 步
   是在 20 多次动量清零下跑出来的，实际有效训练量还要打折。
2. **sub-talker 欠训导致「不清晰」**：码本 1-15 承载声学细节，
   `--sub_talker_loss_weight` 默认 0.3。若「断」修好了但仍发闷，
   可以把这个权重调高再跑一段观察。

**「断」和「不清晰」大概率是两个不同的原因** —— 前者看权重稳定性，
后者看 sub-talker。修一个不一定同时好。
